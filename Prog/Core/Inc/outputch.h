...
{
public:
	OutputCh(GPIO_TypeDef* GPIOx, uint8_t GPIO_Pin, TIM_HandleTypeDef* PWMtim, status *data, uint8_t chNum_); 	//конструктор при создании определяется только пинами, за которые он отвечает, указателем на класс статус и порядковым номером канала
