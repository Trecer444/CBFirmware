...
void TimerInit(void)
{
  ...
  blt_int32u hclk_frequency;
  blt_int32u pclk_frequency;
  blt_int32u pclk_tim_frequency;

  /* All STM32H7 derivatives support a TIM1 peripheral. Its free running counter will be
  ...
  hclk_frequency = LL_RCC_CALC_HCLK_FREQ(SystemCoreClock, LL_RCC_GetAHBPrescaler());
  pclk_frequency = LL_RCC_CALC_PCLK2_FREQ(hclk_frequency, LL_RCC_GetAPB2Prescaler());
